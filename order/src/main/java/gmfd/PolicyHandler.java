...
public class PolicyHandler{
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void onKitchenCancelled(@Payload Kitchencancelled kitchencancelled) {
        try {
            if (kitchencancelled.isMe()) {
                System.out.println("##### listener : " + kitchencancelled.toJson());
                Optional<Order> orderOptional = orderRepository.findById(kitchencancelled.getOrderid());
                Order order = orderOptional.get();
                order.setStatus("KitchenCancelled");
                ...
                /*orderCancelled.setId(kitchencancelled.getOrderid());
                orderCancelled.setCustomerid(order.getCustomerid());
                orderCancelled.setFoodcaltalogid(order.getFoodcaltalogid());
                orderCancelled.setQty(order.getQty());*/
                orderCancelled.setId(kitchencancelled.getOrderid());
                orderCancelled.setCustomerid(order.getCustomerid());
                orderCancelled.setFoodcaltalogid(order.getFoodcaltalogid());
                orderCancelled.setQty(kitchencancelled.getAmount());
                orderCancelled.setStatus("Order Canceled");
                orderCancelled.publish();
